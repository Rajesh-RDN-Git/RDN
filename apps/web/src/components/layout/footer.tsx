      ...
      <div className="mx-auto max-w-content px-4 py-12">
        ...
        <div className="grid grid-cols-2 gap-8 md:grid-cols-4">
          {Object.entries(footerLinks).map(([title, links]) => (
            <div key={title}>
              <h3 className="mb-4 text-label-md font-semibold text-white">{title}</h3>
              <ul className="space-y-3">
                {links.map((link) => (
                  <li key={link.label}>
                    <Link
                      href={link.href}
                      className="text-body-sm text-gray-400 transition-colors hover:text-gray-200"
                    >
                      {link.label}
                    </Link>
                  </li>
        ...
        <div className="mt-12 flex flex-col items-center justify-between gap-4 border-t border-gray-800 pt-8 md:flex-row">
          ...
          <div className="flex items-center gap-4">
            <Link href="/privacy" className="text-body-sm text-gray-500 hover:text-gray-300">
              Privacy Policy
            </Link>
            <Link href="/terms" className="text-body-sm text-gray-500 hover:text-gray-300">
              Terms of Service
            </Link>
            {/* Social icons */}
