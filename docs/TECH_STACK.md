...
> - [Handover code map](./handover/02-CODE-MAP.md)

---

## Installed versions (read from `package.json` and `.nvmrc`, 2026-07-22)

The sections below explain _why_ each technology was chosen and what was rejected. That
reasoning is still accurate. For _what is actually installed_, use this table — the prose
below describes some choices loosely ("Next.js 14+").
...
| Web                      | Next.js App Router, React, Tailwind, react-hook-form, socket.io-client               | Next 14.2.22, React 18.3.1, Tailwind 3.4.17                             |
| Mobile                   | Expo (managed) + Expo Router, React Native, Zustand                                  | Expo SDK 54.0.33, React Native 0.81.5, React 19.1.0, expo-router 6.0.23 |
| API                      | NestJS, Prisma, Zod + class-validator, socket.io, ioredis, helmet, @nestjs/throttler | NestJS 10.4.15, Zod 3.24.1, helmet 8.2.0, throttler 6.5.0               |
| Data                     | PostgreSQL 16.14 (RDS), Redis (ElastiCache)                                          |                                                                         |

### ⚠️ Web is on React 18, mobile is on React 19

Both live in one pnpm workspace and only one can win the hoisted root slot — the lockfile
decides, not the filter flag. This is why the AWS Amplify build spec deletes `apps/mobile`
and `pnpm-lock.yaml` before installing and then removes the nested `react` and `react-dom`
from `apps/web/node_modules`. Without those steps the bundle carries two React instances and
the site crashes on `/_error`. Do not "simplify" that build spec — see
[handover/07](./handover/07-KNOWN-ISSUES-AND-GOTCHAS.md).

### ⚠️ Search is not full-text yet

The PostgreSQL section below cites full-text search with GIN indexes and PostGIS as a reason
for choosing Postgres. That remains the design intent, but as of 2026-07-22 it is **not
implemented**: `apps/api/src/modules/search/search.service.ts` uses Prisma `contains` with
`mode: 'insensitive'`, which compiles to `ILIKE '%term%'` and cannot use an index, and the
Prisma schema defines no `tsvector`, GIN, or PostGIS index. Search is correct and fast
enough at current volume; it becomes a full table scan problem in the low thousands of
listings. Tracked as P2-2 in the
[handover backlog](./handover/06-BACKLOG-PRIORITY-IMPACT.md).
