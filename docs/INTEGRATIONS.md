...
> - [PRD](./PRD.md)
> - [Handover — access and credentials](./handover/04-ACCESS-AND-CREDENTIALS.md)
...
## Live status (verified against the production ECS task definition, 2026-07-22)

The setup instructions below are accurate. This table records which integrations are
actually **wired into production** — several are fully coded and completely inert because
no credentials are injected.

| Integration                     | Production status              | Note                                                                                                                                                                                                                       |
| ------------------------------- | ------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **MSG91** (phone OTP)           | ✅ Live and verified           | Real handsets receive OTPs. Note the per-auth-key IP allowlist — the NAT elastic IP `13.204.206.193` is whitelisted.                                                                                                       |
| **AWS S3 + CloudFront** (media) | ✅ Live                        | Authenticates via the **ECS task role**, not static keys.                                                                                                                                                                  |
| **Exotel** (masked calling)     | ⚠️ Credentials set, calls fail | Blocked on **Exotel KYC completion**. Every call returns "account not KYC compliant".                                                                                                                                      |
| **Razorpay** (payments)         | ❌ Not configured              | `RAZORPAY_KEY_ID` and `RAZORPAY_KEY_SECRET` are present but empty; `RAZORPAY_WEBHOOK_SECRET` is absent entirely. Signature verification and idempotency are also not implemented — build them before any real money moves. |
| **Firebase / FCM** (push)       | ❌ Not configured              | No `FIREBASE_PROJECT_ID` or `FIREBASE_SERVICE_ACCOUNT` in the task definition. Push notifications do not work in production.                                                                                               |
| **AWS SES** (email)             | ❌ Not configured              | No `AWS_SES_FROM_EMAIL` in the task definition.                                                                                                                                                                            |
| **Sentry** (error tracking)     | ❌ Not configured              | No `SENTRY_DSN` in the task definition. PII scrubbing is implemented and waiting for a DSN. Production currently has no error visibility.                                                                                  |
| **Interakt / Wati** (WhatsApp)  | ❌ Not configured              |                                                                                                                                                                                                                            |

Remediation for each is tracked in the
[handover backlog](./handover/06-BACKLOG-PRIORITY-IMPACT.md).

> **Communication constraint.** Never configure any integration — SES sender or recipient,
> SNS subscription, alert target, or application notification — to an address at
> `workctrl.tech` or `sworks.co.in`. All alerting goes to rajesh@rdngroups.com.
